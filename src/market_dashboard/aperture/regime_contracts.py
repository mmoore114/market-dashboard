...
from market_dashboard.aperture.leadership_contracts import ResearchUniverseV1, StrengthSourceV1, LeadershipOutputV1
...
class SpotVolatilityIdentityV1(ContractModel):
    identity_version: Literal['spot-volatility-identity-v1'] = 'spot-volatility-identity-v1'
    canonical_series: Literal['$VIX'] = '$VIX'
    disposition_version: Literal['deepvue-identity-disposition-v1'] = 'deepvue-identity-disposition-v1'
    ...
    @field_validator('source_symbol','data_vendor','dataset_id','equivalence_evidence')
    @classmethod
...
class RegimeInputV1(ContractModel):
    ...
    def alignment(self):
        ...
        if sorted(i.symbol for i in self.breadth)!=sorted(self.universe.symbols):
            raise ValueError('Exactly one breadth row per dated research member required')
        p = self.universe.provenance
        if not p.effective_session<=self.session_date<=p.valid_through:
            raise ValueError('Universe is not valid as of T')
        if self.structure is not None:
            c = self.structure
            if c.session_date!=self.session_date or c.source!=self.source or c.universe!=self.universe:
                raise ValueError('Structure context session/source/universe mismatch')
            symbols = [e.inputs.symbol for e in c.evidence]
            if len(set(symbols))!=len(symbols) or not set(symbols)<=set(self.universe.symbols):
            ...
            if any(e.inputs.session_date!=self.session_date or e.inputs.source.model_dump()!=basis or
                   e.rules_fingerprint!=structure_rules for e in c.evidence):
                raise ValueError('Structure evidence date/source/version mismatch')
        if self.leadership is not None:
            c = self.leadership
            if c.session_date!=self.session_date or c.source!=self.source or c.universe!=self.universe or c.calendar_fingerprint!=self.calendar_fingerprint or c.rules_fingerprint!=leadership_rules:
                raise ValueError('Leadership session/source/universe/calendar/version mismatch')
            if sorted(e.inputs.symbol for e in c.symbols)!=sorted(self.universe.symbols):
                raise ValueError('Leadership population mismatch')
            if any(e.inputs.session_date!=self.session_date or e.inputs.source!=self.source or e.universe!=p or
                   e.universe_policy_version!=self.universe.policy_version for e in c.symbols):
                raise ValueError('Leadership symbol evidence mismatch')
            keys = [(g.group_type,g.group_id) for g in c.groups]
            if len(keys)!=len(set(keys)) or any(g.session_date!=self.session_date or not
                g.membership.effective_session<=self.session_date<=g.membership.valid_through for g in c.groups):
                raise ValueError('Duplicate or incorrectly dated group evidence')
            sub_members = [m.source_symbol for g in c.groups if g.group_type=='SUB_INDUSTRY'
...
class SleeveV1(ContractModel):
    ...
    def score_matches_state(self):
        if self.score != {State.GREEN:1, State.YELLOW:0, State.RED:-1}.get(self.state):
...
class FractionV1(ContractModel):
    ...
    def denominator_consistency(self):
        ...
        coverage = self.valid_count/self.population_count if self.population_count else 0.
...
class RegimeMemoryV1(ContractModel):
    ...
    def memory_consistency(self):
        if self.confirmed_state is None:
            if self.entered_date is not None or self.confirmed_sessions_in_state or self.candidate_streak or self.candidate:
        ...
        if (self.candidate is None)!=(self.candidate_streak==0) or self.candidate_streak>1:
...
class RegimeOutputV1(ContractModel):
    ...
    threshold_version: Literal['market-regime-thresholds-v1'] = 'market-regime-thresholds-v1'
