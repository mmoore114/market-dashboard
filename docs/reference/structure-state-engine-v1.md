...
| State | Job |
|---|---|
| `BASE` | No confirmed directional trend. Price is digesting. |
...
- `RANGE` is not a separate state. Dead sideways names and constructive consolidations are both `BASE`. Leadership, compression, and setup engines decide whether the base is interesting.
...
- Transitional states `EMERGING` and `DETERIORATING` stay. Inflection is the part swing traders actually use. Collapsing them into `BASE`/`UPTREND`/`DECLINE` would hide the exact window where most swing decisions happen.
...
`BASE` is the residual. That is intentional. Anything that is not a forming, confirmed, weakening, or down trend is digestion.

Priority if multiple candidates fire (should be rare):

```
DECLINE > UPTREND > DETERIORATING > EMERGING > BASE
```
...
| from \ to        | BASE | EMERGING | UPTREND | DETERIORATING | DECLINE |
|------------------|:----:|:--------:|:-------:|:-------------:|:-------:|
| BASE             |  Y   |    Y     |    S    |       —       |    S    |
| EMERGING         |  Y   |    Y     |    Y    |       Y       |    S    |
| UPTREND          |  —   |    —     |    Y    |       Y       |    S    |
...
- `UPTREND` cannot go directly to `BASE`. It must pass through `DETERIORATING` unless a crash shock sends it to `DECLINE`.
- `UPTREND` cannot go to `EMERGING`. Emerging is a birth state, not a demotion.
- `BASE` cannot go to `DETERIORATING`. There is no up-structure to deteriorate.
- `DECLINE` cannot go to `DETERIORATING`. Deteriorating is defined as a damaged *up* structure.
- `BASE → UPTREND` is shock-only. Gradual births go `BASE → EMERGING → UPTREND`.
- `DECLINE → EMERGING` is shock-only (violent reversal). Normal repair is `DECLINE → BASE → EMERGING`.
...
Classification is `f(measurements, previous_state, cand_streak)`, never `f(measurements)` alone. A stock hugging SMA50 from `UPTREND` stays `UPTREND` until `BROKEN_UP` persists. The same tape arriving from `BASE` stays `BASE` or becomes `EMERGING`.
...
```
{
  ...
  "engine_version": "structure-engine-v1.0",
...
**12. Bases** — all non-trend digestion is `BASE`, including:  
...
|---|---|
| BASE | range breakout, early coil, failed breakdown reclaim |
...
Illegal pairings the workstation can warn on: `DECLINE + trend pullback`, `BASE + “in-trend pullback”`, `UPTREND + breakdown` (should already have moved).
...
Yesterday `BASE`. Today `gap_atr = +3.1`, close above SMA20 and SMA50, `dist50 = 1.4`.  
...
1. Five states: `BASE`, `EMERGING`, `UPTREND`, `DETERIORATING`, `DECLINE`.
...
|---|---|---|
| Flat base | C = 50 ± 0.4 ATR noise, flat MAs, 60 days | Stay `BASE`. No EMERGING. |
| Gradual emergence | SMA50 slowly lifts, price holds above it for 8–12 days | `BASE` → `EMERGING` (≥2d) → `UPTREND` (≥3d). No skip. |
...
| EP gap | From BASE, +3 ATR gap, hold | Same-day `EMERGING` or `UPTREND` via shock. |
| Crash gap | From UPTREND, −3 ATR gap under SMA50 | Same-day `DECLINE`. |
| Noisy oscillator | C crosses SMA20/SMA50 every 2–4 days, s50 ≈ 0 | Stay `BASE`. Persistence must block flicker. |
| Illegal edge attempt | Force candidate DECLINE while state is BASE without shock | Stay `BASE`. Log blocked transition. |
...
3. Hand-label 30–50 charts *by date* (not by eventual winner): “on this Friday, I would have called this UPTREND / BASE / …” Include failures and chop. Measure agreement and, more important, measure *flip rate*.
...
- Seed `previous_state = BASE` on first eligible day.
...
- Do not let NaNs silently become BASE. If ATR or SMA50 is missing, emit `state = null` and `error = insufficient_history`.
