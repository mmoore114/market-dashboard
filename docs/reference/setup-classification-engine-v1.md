...
Version: `setup-engine-v1.0`  
Horizon: daily OHLCV, 1–6 week swing context  
Companion: `structure-engine-v1.0` (states: `BASE`, `EMERGING`, `UPTREND`, `DETERIORATING`, `DECLINE`)

This brief calls the non-trend structure state `NEUTRAL`. In the shipped structure engine that state is `BASE`. Treat them as the same cell: `BASE ≡ NEUTRAL`. Compatibility matrices use the structure-engine names.
...
SetupObject {
  ...
  # lifecycle
  status                    # FORMING | NEAR_TRIGGER | TRIGGERED | FAILED | STALE
  detected_at
  status_changed_at
  age_sessions
  sessions_in_status
...
| `STALE` | Useful window expired without a current trigger, or geometry changed enough that this instance is no longer the same setup. Terminal. |

Rules:

- A new instance is born `FORMING` or `NEAR_TRIGGER` or `TRIGGERED` (EP is usually born `TRIGGERED`).
- `TRIGGERED` does not expire into `FORMING`. It goes `STALE` or `FAILED`.
- Failed and stale instances are kept on the daily snapshot for history but are not “active.” Active = `{FORMING, NEAR_TRIGGER, TRIGGERED}`.
...
**Transition vs continuation** is not a subtype. Store `structure_state` on the object. Policy can treat `BASE/DECLINE + EP` differently from `UPTREND + EP`.
...
| `TRIGGERED` window | sessions 0–5 after event (**P** `ep_live=5`) while not failed |
| `FAILED` LONG | Close < min(event_day Low, event_day Open) − 0.25 ATR (**P**) |
| `STALE` | session > `ep_live` and not failed |
...
```
DEF  pivot = max(High[t-9 : t])          # HH10, inclusive
...
**SHORT** uses `pivot = min(Low[t-9:t])`, location near the top of the box (`Close <= HH20 - 0.35*Range20`), trigger `Close < pivot - 0.10 ATR`.
...
**Do not require** `structure == UPTREND`. A coil can exist in `BASE` or `EMERGING`. Compatibility is recorded, not enforced as a voter except for direction sanity (see matrix).
...
| setup \ structure | BASE/NEUTRAL | EMERGING | UPTREND | DETERIORATING | DECLINE |
|---|:---:|:---:|:---:|:---:|:---:|
| EP LONG | ✓ | ✓ | ✓ | ✓ | ✓ |
| EP SHORT | ✓ | ✓ | ✓ | ✓ | ✓ |
...
```
{
  ...
  "structure_engine_version": "structure-engine-v1.0",
  ...
  "engine_version": "setup-engine-v1.0",
...
### EP-1 — valid long EP from BASE

Yesterday close 40.00, ATR14=1.00. Today open 42.20 (GapATR=+2.20), high 43.10, low 41.90, close 42.85, RVOL20=2.4, CLV=0.79, Close>Open. Structure=`BASE`.  
...
Each with `direction`, a five-state lifecycle, ATR-normalized geometry, and a persisted evidence object.
